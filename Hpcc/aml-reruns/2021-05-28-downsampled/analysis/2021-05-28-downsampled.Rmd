...
working_directory <- "Hpcc/aml-reruns/2021-05-28-downsampled/analysis/"
...
```{r}
ggplot(data, aes(x=evaluations, y=ele_agg_per, color=proportion)) +
...
elite_ave_performance_fig <-
  ggplot(
    data,
    aes(x=evaluations, y=elite_trait_avg, color=proportion, fill=proportion)
  ) +
  stat_summary(geom="line", fun=mean) +
  stat_summary(
    geom="ribbon",
...
```{r}
ggplot(data, aes(x=evaluations, y=uni_str_pos, color=proportion)) +
  stat_summary(geom="line", fun=mean) +
  stat_summary(
    geom="ribbon",
    fun.data="mean_cl_boot",
...
unique_start_position_coverage_fig <- ggplot(
    data,
    aes(x=evaluations, y=unique_start_positions_coverage, color=proportion, fill=proportion)
  ) +
  stat_summary(geom="line", fun=mean) +
  stat_summary(
    geom="ribbon",
...
unique_start_positions_coverage_final_fig <- ggplot(
    final_data,
    aes(x=proportion, y=unique_start_positions_coverage, fill=proportion)
  ) +
  geom_flat_violin(
    position = position_nudge(x = .2, y = 0),
    alpha = .8,
